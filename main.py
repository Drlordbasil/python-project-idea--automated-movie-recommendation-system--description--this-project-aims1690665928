...
class MovieRecommender:
    def __init__(self, movie_data_file):
        ...
        self.count_matrix = self.count_vectorizer.fit_transform(self.movies['plot'])
        self.cosine_similarities = cosine_similarity(self.count_matrix, self.count_matrix)

    def find_movie_index(self, movie_title):
        movie_indices = self.movies.loc[self.movies['title'].str.lower() == movie_title.lower()].index
    ...
    def find_similar_movies(self, query_movie, num_recommended_movies=5):
        ...
        top_movies = [self.movies.iloc[sim_score[0]]['title'] for sim_score in sim_scores[1: min(num_recommended_movies + 1, len(sim_scores))]]
...
if __name__ == "__main__":
    ...
    recommendations = movie_recommender.find_similar_movies(query_movie, num_recommended_movies=5)
